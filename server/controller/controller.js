...
exports.register = async (req, res) => {
  try {
    ...
    // generate API Key
    let genAPIKey = generateApiKey({ method: 'uuidv4', dashes: false })
...
exports.apikey = async (req, res) => {
  ...
  data.forEach(element=> {
    if(key !== element.apikey) {
      return hint = "not Match";
    }else {
      return hint = "match";
    }
  })
  ...
  if(hint === "match") {
    response = await Image.find().limit(10);
  }else {
    response = "Invalid API_KEY";
  }
  res.json({response});
};
